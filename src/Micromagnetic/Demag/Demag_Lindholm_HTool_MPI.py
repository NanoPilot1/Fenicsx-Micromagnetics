...
# Split application arguments from PETSc options
...
# Optional dense validation assembly
...
def _fill_lindholm_block_incident_numba(
    ...
    for ii in prange(M):
        ...
        for jj in range(N):
            ...
            if gi == gj:
                out[ii, jj] += omega_sum[gi] * inv4pi - 1.0
...
class DemagField_Lindholm_HTool_MPI:
    def __init__(
        ...
        # Dirichlet boundary data as a DOLFINx Function.
        # This is safer under MPI than using a standalone PETSc Vec here.
        ...
        # Memoria teórica de la matriz densa completa B.
    ...
    def _set_boundary_x_from_u1(self):
        ...
        x_full = np.zeros(self.Nb, dtype=np.float64)
    ...
    def apply_B_to_u1_boundary(self):
        ...
        # Store boundary Dirichlet data in the natural DOLFINx Function layout.
        # Do not call Vec.assemble() here. scatter_forward() is the correct
        # DOLFINx operation after local array edits.
        self.g_u2_fun.x.array[:] = 0.0
    ...
    def validate_htool_against_dense(self, seed=1234):
        """
        Validation against local dense owned-row assembly. Use only for small meshes.
        """
