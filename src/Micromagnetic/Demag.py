...
def build_nullspace_per_component(mesh, V1, A_u1):
    ...
    mesh.topology.create_connectivity(tdim, fdim)
    mesh.topology.create_connectivity(fdim, tdim)
    mesh.topology.create_connectivity(tdim, 0)   # cell -> vertex
    mesh.topology.create_connectivity(0, tdim)   # vertex -> cell
    ...
    assert ns.test(A_u1), "Nullspace fails test(A). Likely: non-global components in MPI."
