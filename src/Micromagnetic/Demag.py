...
class DemagField:
    def __init__(self, domain_mesh, V, V1, Ms):
        ...
        pc.setType("gamg")
        pc.setReusePreconditioner(True)   # esto es “el paso 5”
    ...
    def solve_u2(self):
        ...
        if self.rank == 0:
            ...
            #phi_vals_all = np.concatenate(phi_vals_all)
            #phi_serial_vals = phi_vals_all[self.indices]
            ###########################################################
            pos = 0
    ...
    def Energy(self, m):
        ...
        integrand = ufl.inner(m, self.H_d) * dx
        energy = fem.assemble_scalar(fem.form(integrand))
