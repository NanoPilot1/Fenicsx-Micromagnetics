class DMIBULK:
    ...
    def Energy(self, m):
        ...
        energy = 0.5 * self.mu0 * self.M_s *fem.assemble_scalar(fem.form(integrand))
        #curl_m = ufl.curl(m)  
        #integrand2 = self.D * ufl.inner(m, curl_m) * ufl.dx
        #E_dmi = fem.assemble_scalar(fem.form(integrand2))
        return energy  *1e-27
