...
class StopByMaxDmdtFD:
    """
    Estima max|dm/dt| (deg/ns) por diferencia finita entre pasos ACEPTADOS.
    No evalúa H_eff. No depende de ydot.
    """
...
class LLG:
    ...
    def _run_ts(self):
        """Corre TS.solve(self.y) y sincroniza hef.m al final."""
        ts = self.ts
        hef = self.hef
        y = self.y
