...
class DMIInterfacial:
    def __init__(self, mesh, V, V1, D, n0, Ms, VolN):
        ...
        self.e0 = ufl.as_vector([1, 0, 0])
        self.e1 = ufl.as_vector([0, 1, 0])
        self.e2 = ufl.as_vector([0, 0, 1])

        self.total_size = self.H_DMI.x.array.shape[0]

        self.n0, self.n1, self.n2 = ufl.split(self.n) 
        self.vec_total = np.zeros((self.total_size // self.dim, self.dim))

        self.view_rhs = self.rhs_func.x.array.reshape(-1, self.dim)
        self.view_H = self.H_DMI.x.array.reshape(-1, self.dim)
    ...
    def Energy(self, m):
        ...
        integrand = ufl.inner(m, self.H_DMI) * ufl.dx
        energy = -0.5 * self.mu_0 * self.Ms * fem.assemble_scalar(fem.form(integrand)) * 1e-27
        return energy
