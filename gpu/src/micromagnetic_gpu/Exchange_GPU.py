...
class ExchangeField:
    ...
    def __init__(self, mesh, V, A, Ms, VolN):
        ...
        prefactor.x.array[:] = (-2.0 * self.A / (self.mu_0 * self.M_s) / VolN[:] / 1e-18)
    ...
    def Energy(self, m):
        ...
        """
        E_int = ufl.inner(ufl.grad(m), ufl.grad(m)) * ufl.dx
        energy = self.A * fem.assemble_scalar(fem.form(E_int)) * 1e-9
