...
"""
GPU interfacial Dzyaloshinskii-Moriya interaction field.

The interfacial DMI operator is assembled with DOLFINx, scaled by the lumped
nodal volume, and converted to a PETSc AIJCUSPARSE matrix for CUDA execution.
...
class DMIInterfacial:
    ...
    where n is the interface normal field. A boundary contribution is
    ...
    def __init__(self, mesh, V, V1, D, n0, Ms, VolN):
        ...
        prefactor.x.array[:] = (
            -2.0 * self.D / (self.mu_0 * self.Ms) / 1e-9 / VolN[:]
        )
    ...
    def Energy(self, m):
        ...
        """
        E_int = ufl.inner(m, self.H_DMI) * ufl.dx
        energy = -0.5 * self.mu_0 * self.Ms * fem.assemble_scalar(fem.form(E_int))
